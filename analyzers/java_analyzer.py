...
def analyze_java(repo_url):
    """
    Analyze a Java project by fetching its pom.xml or
    build.gradle and analyzing the project structure,
    ...
    if check_file_existence_in_repo(repo_url, "pom.xml"):
        ...
        if build_file_content:
            ...
            for dependency in dependencies:
                group_id = (
                    dependency.find("{http://maven.apache.org/POM/4.0.0}groupId").text
                    if dependency.find("{http://maven.apache.org/POM/4.0.0}groupId")
                    is not None
                    else ""
                )
                artifact_id = (
                    ...
                    if dependency.find("{http://maven.apache.org/POM/4.0.0}artifactId")
                    is not None
                    else ""
                )
        ...
        if build_file_content:
            ...
            if (
                "org.jetbrains.kotlin" in build_file_content
                or 'kotlin("jvm")' in build_file_content
            ):
                analysis_result["languages"].append("Kotlin")
    ...
    if not analysis_result["dependency_manager"]:
        return {
            "error": "Project build system could not be determined or is unsupported."
        }
