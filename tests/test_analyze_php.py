...
class TestAnalyzePHP(unittest.TestCase):
    ...
    def test_php_framework_and_docker_detection(
        self, mock_check_file_existence, mock_fetch_file_from_github
    ):
        # mocking fetch_file_from_github to simulate composer.json with Laravel framework
        composer_json_content = json.dumps({"require": {"laravel/framework": "^8.0"}})
        ...
        mock_fetch_file_from_github.assert_called_once_with(
            "https://github.com/some/php_repo", "composer.json"
        )
        mock_check_file_existence.assert_any_call(
            "https://github.com/some/php_repo", "Dockerfile"
        )
        mock_check_file_existence.assert_any_call(
            "https://github.com/some/php_repo", "package.json"
        )
